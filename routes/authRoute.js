...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newUserProfile = new UserProfiles({ user: newUser._id });
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.json({ message: 'Login successful!', username: user.username, hasProfile });
    } catch (error) {
...
router.get('/check-auth', csrfProtection, async (req, res) => {
  ...
  if (token) {
    try {
      ...
      const hasProfile = !!userProfile.firstname && !!userProfile.lastname && !!userProfile.bio;
      res.status(200).json({ authenticated: true, username: decoded.username, hasProfile });
    } catch (err) {
