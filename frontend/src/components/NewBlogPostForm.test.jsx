...
describe("NewBlogPostForm", () => {
  ...
  test("renders the form with title and content fields and a submit button", () => {
    render(<NewBlogPostForm onPostAdded={onPostAddedMock} />);

    // Check for heading text
    expect(screen.getByText(/Add a New Blog Post/i)).toBeInTheDocument();
  ...
  test("calls onPostAdded with sanitized data when form is submitted", async () => {
    ...
    await waitFor(() => {
      ...
      // Verify onPostAdded is called with sanitized data
      expect(onPostAddedMock).toHaveBeenCalledWith({
  ...
  test("alerts when either title or content is missing", () => {
    ...
    expect(onPostAddedMock).not.toHaveBeenCalled();
